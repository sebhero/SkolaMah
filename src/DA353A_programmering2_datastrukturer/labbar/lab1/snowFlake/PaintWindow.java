...
public class PaintWindow extends JFrame {
    ...
    public PaintWindow() {
        this(600,400,Color.WHITE);
    }
    
    public PaintWindow(int width, int height, Color background) {
    ...
    public PaintWindow(ImageIcon background) {
        ...
        if(background==null) {
            ...
            setUpWindow(Math.min(background.getIconWidth(),800),
                        Math.min(background.getIconHeight(),800));
        }
    }
  
    private void setUpWindow(int width, int height) {
        this.setTitle("Paint Window");
