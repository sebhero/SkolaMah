...
//		int doesItExist = lists[index].indexOf(value);
//		if (doesItExist == -1) {
//			return false;
//		} else {
//			return lists[index].remove(doesItExist) != null ? true : false;
//		}
		try {
			result = lists[index].remove(lists[index].indexOf(value)) != null ? true : false;
		} catch (Exception e) {
...
//        System.out.println("Tar bor 'Excellent': " + values.remove("Excellent"));
	...
	private int size() {
		int size = 0;
		for (LinkedList<String> item : lists) {
			size += item.size();
		}
		return size;
	}
